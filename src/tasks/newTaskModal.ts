...
export class NewTaskModal extends Modal {
	...
	constructor(app: App, plugin: TaskToolsPlugin, preselectedChain?: ChainDefinition) {
		...
		for (const chain of plugin.taskSettings.chains) {
	...
	onOpen(): void {
		...
		for (const chain of this.plugin.taskSettings.chains) {
	...
	private resolveCreationConfig(): {
		...
		const enrolledWithConfig = this.plugin.taskSettings.chains.filter((c) => {
		...
		return {
			folder: source?.itemFolder ?? this.plugin.taskSettings.taskFolder,
			templatePath: source?.itemTemplatePath ?? this.plugin.taskSettings.taskTemplatePath,
			fmKey: source?.itemFrontmatterKey ?? this.plugin.taskSettings.taskFrontmatterKey,
			fmValue: source?.itemFrontmatterValue ?? this.plugin.taskSettings.taskFrontmatterValue,
		};
	}
